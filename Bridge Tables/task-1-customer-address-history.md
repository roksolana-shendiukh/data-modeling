...
- `bridge_membership_history` is a second, independent bridge – this one between `dim_member` and `dim_pricing_tier` – using `start_date`/`end_date` to implement SCD Type 2 on the membership relationship itself, separate from the reservation lifecycle entirely.
...
- **Pricing tier -> its own bridge (`bridge_membership_history`), Type 2**: a member's tier changes over time and needs to be reconstructable at any point (e.g., "what was their monthly fee when they signed up"), same reasoning as the address-history bridge – don't force it to piggyback on `dim_member`.
